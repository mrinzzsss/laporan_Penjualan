        ...
        @foreach($kategoriList as $kategori)
            ...
            <div class="mb-5">
                ...
                <div class="flex gap-3 overflow-x-auto pb-2" style="scroll-snap-type: x mandatory;">
                    ...
                    <div class="flex-none border border-slate-200 rounded-lg overflow-hidden relative"
                         style="width: calc((100% - 4 * 12px) / 5); min-width: 150px; scroll-snap-align: start;">
                        <button type="button"
                                onclick="bukaModalMenu({{ $barang->id }}, '{{ $barang->nama }}', {{ $barang->harga }})"
                                class="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-sky-400 hover:bg-sky-500 text-white text-sm leading-none">
                            +
                        </button>
                        @if($barang->gambar_url)
                            <img src="{{ $barang->gambar_url }}" alt="{{ $barang->nama }}" class="w-full h-24 object-cover bg-slate-100">
                        @else
                            <div class="w-full h-24 flex items-center justify-center bg-slate-100 text-slate-400 text-xs">Tidak ada gambar</div>
                        @endif
                        <div class="p-2">
                            <div class="text-xs font-semibold text-slate-800">{{ $barang->nama }}</div>
                            <div class="text-xs text-slate-500 mb-1" style="min-height: 28px;">{{ $barang->deskripsi }}</div>
