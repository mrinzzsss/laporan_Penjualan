...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-800">Tambah Barang</h1>
        <p class="text-sm text-slate-500">Isi detail barang baru.</p>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 p-6 max-w-2xl">
        <form method="POST" action="{{ route('barang.store') }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="deskripsi" value="{{ old('deskripsi') }}" placeholder="opsional, tampil di card menu"
                       class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-sky-400">
            ...
            <div>
                ...
                <div id="preview-container" class="hidden mb-3">
                    <img id="image-preview" src="#" alt="Preview Gambar" class="w-24 h-24 rounded-lg object-cover border border-slate-200 shadow-sm">
                </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" checked class="rounded border-slate-300 text-sky-400 focus:ring-sky-400">
